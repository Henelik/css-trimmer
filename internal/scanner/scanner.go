...
// findClassNameMatch is a faster version of FindAllStringSubmatch with the regex `class="([^"]*)"`
// Returns only the submatches, and not the outer matches.
func findClassMatch(content string) []string {
	idx := strings.Index(content, classStart)
	if idx == -1 {
		return nil
	}
	...
	for {
		// move the index to the beginning of the substring
		idx += 7
		...
		// offset the end index, since we searched from the index
		endIdx += idx

		result = append(result, content[idx:endIdx])

		// search for another match, starting after the end of the previous match
		idx = strings.Index(content[endIdx+1:], classStart)
		if idx == -1 {
			break
		}

		// offset the index, since we searched from the end of the previous match
		idx += endIdx + 1
	}
